...
class GxHtml extends CHtml {
	 ...
	 */
	public static function listDataEx($models, $valueField = null, $textField = null, $groupField = '', $textField2=null) {
		$listData = array();
		if ($groupField === '') {
			foreach ($models as $model) {
				...
				if ($textField2) $text .=" ( " . self::valueEx($model, $textField2) . ")";
				$listData[$value] = $text;
			}
		} else {
			foreach ($models as $model) {
	...
	public static function linkButton($label='button', $url = "#", $htmlOptions=array(), $icon="") {
	  
	  if (empty($htmlOptions["class"])) $htmlOptions["class"] = "but blue tiny";
	  $htmlOptions["role"] = "button";	  
	  if (!empty($icon)) { $htmlOptions["class"] .= " ui-button-text-icon-primary"; $span = CHtml::tag('span',array("class" => $icon . " ui-button-icon-primary ui-icon"));	  }
	  $text = $span . CHtml::tag('span', array("class" => "ui-button-text"), $label);
	  $htmlOptions["href"] = $url;
	  $htmlOptions['id'] = isset($htmlOptions['id']) ? $item['htmlOptions']['id'] . rand() : 'am' . uniqid();
	  if (!empty($htmlOptions["ajax"])) $htmlOptions["ajax"]["type"] = "POST";
	  if (empty($htmlOptions["ajax"]["update"])) { $htmlOptions["class"] .= " update-dialog-open-link ";
	    if (empty($htmlOptions["data-update-dialog-title"])) $htmlOptions["data-update-dialog-title"] = $label; 
	    return CHtml::link($text, CHtml::encode($url), $htmlOptions); 
	  } else {
	    $ajax_options = $htmlOptions["ajax"];

	    $jquery_method = '$("' . $htmlOptions["ajax"]['update'] . '").html(data);';
	    
	  }
	

	    return CHtml::ajaxLink($text, CHtml::encode($url),$ajax_options, $htmlOptions);
	...
	public static function submitButton($label='submit',$htmlOptions=array())
	{
		...
		$htmlOptions['class'] = 'button';
		$htmlOptions['data-icon-primary'] = 'ui-icon-circle-check';

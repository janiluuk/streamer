...
var updateDialog = {
   ...
   */
  addLoader : function(){
	this.dialogContent.html("").addClass("loading");
  },
  ...
  init : function(name){
    ...
    if (!this.defaultWidth) this.defaultWidth = this.dialog.dialog( 'option', 'width' );
    ...
    $( '.update-dialog-content input[type=submit]' )
      ...
      $('.ui-dialog-titlebar-close').click(function() { setTimeout(function() { $('.update-dialog-content').html(''); }, 500); });
  ...
  removeLoader : function(){
	this.dialogContent.html("").removeClass("loading").empty();
  },
  ...
  'success': function( data ){
        ...
        updateDialog.removeLoader();
	  if (data.notify != "") {
              $("#notifications").html('<div class="message '+data.status+'"><p>'+data.notify+'</p></div>');
	        }
	console.log(data);
      // Add returned contents to UpdateDialog
        //	updateDialog.dialogContent.html( data.content );
        ...
        updateDialog.callback( data.status );
				if (data.status == "reload") {
					var div = data.div;
					$("#notifications").html('<div class="message success"><p>'+data.content+'</p></div>');
					var url = data.url;
					$(div).load(url);		
				}

				if (data.status == "reloadtab") {
...
function updateDialogOpen( e ){
  ...
  updateDialog.defaultWidth = $( this ).data( 'update-dialog-width' );
  updateDialog.width = $( this ).data( 'update-dialog-width' );
  updateDialog.height = $( this ).data( 'update-dialog-height' );
  // Initialize update dialog if it's the first run
  if( typeof updateDialog.dialog === 'undefined' )
  {
    updateDialog.init("#update-dialog");
    updateDialog.open( $( this ).attr( 'href' ) );
  } else {
    updateDialog.init("#update-dialog-1");
    updateDialog.open( $( this ).attr( 'href' ) );
  }
